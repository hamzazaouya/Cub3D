    ...
    while (rdist >= 0)
    {
        if (window_valid(ray->col, y))
        {
            if (ray->texture_type == NORTH || ray->texture_type == SOUTH)
                my_mlx_pixel_put(game->mlx->img, ray->col, y, get_color_from_texture(game, ray, (int)(roundf(yto)) % CUBE_SIZE, (int)(ray->x_end) % CUBE_SIZE)) ;
            else
                my_mlx_pixel_put(game->mlx->img, ray->col, y, get_color_from_texture(game, ray, (int)(roundf(yto)) % CUBE_SIZE, (int)(ray->y_end) % CUBE_SIZE)) ;
