...
int check_edges(char **map, int width, int height)
{
    ...
    while (map[++i])
    {
        ...
        while (s && s[++j])
        {
            if ((i == height - 1 || i == 0 || j == len - 1 || j == 0) && s[j] != '1')
            {
                free(s) ;
                return (0) ;
            }
...
int check_player_path(char **map, int width, int height, int px, int py)
{
    if (is_space(map[py][px]))
        return (0) ;
    if ((py >= height || py < 0 || px >= ft_strlen(map[py]) || px < 0) || map[py][px] == '1')
        return (1) ;
    if (map[py][px] == '2')
        return (1) ;
    if (map[py][px] == '0')
        map[py][px] = '2' ;
    return (check_player_path(map, width, height, px + 1, py) && check_player_path(map, width, height, px - 1, py) \
    && check_player_path(map, width, height, px, py + 1) && check_player_path(map, width, height, px, py - 1)) ;
}
...
int is_map_correct(char **map)
{
    ...
    if (inclusive(0, (width - 1), x + 1) && is_space(map[y][x + 1]))
        ...
        return (0) ;
    else if (inclusive(0, (height - 1), y - 1) && x < ft_strlen(map[y - 1]) && is_space(map[y - 1][x]))
        return (0) ;
    else if (inclusive(0, (height - 1), y + 1) && x < ft_strlen(map[y + 1]) && is_space(map[y + 1][x]))
        return (0) ;
    return (1) ;
...
int is_map_works_fine(t_game *c, t_mq *head)
{
    ...
    if (!check_edges(c->map, get_map_width(c->map), get_map_height(c->map)))
        error(MAP_NOT_VALID_ERROR) ;
    if (!check_player_path(c->map, get_map_width(c->map), get_map_height(c->map), get_player_x(c->map), get_player_y(c->map)))
        error(PLAYER_HAS_PATH_TO_SPACE_ERROR) ;
    replace_spots(c->map, '2', '0') ;
    rm_spaces_from_map(c->map) ; // removing spaces before normalizing the map is the correct thing
    // if (!check_edges(c->map, c->width, c->height))
    //     error(MAP_NOT_VALID_ERROR) ;
